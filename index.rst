.. This file is part of "Présentation du noyau de Redox OS".

..     Copyright (C) 2018 Julien Férard

..     "Présentation du noyau de Redox OS" is free software: you can redistribute it and/or modify
..     it under the terms of the GNU General Public License as published by
..     the Free Software Foundation, either version 3 of the License, or
..     (at your option) any later version.

..     "Présentation du noyau de Redox OS" is distributed in the hope that it will be useful,
..     but WITHOUT ANY WARRANTY; without even the implied warranty of
..     MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
..     GNU General Public License for more details.

..     You should have received a copy of the GNU General Public License
..     along with "Présentation du noyau de Redox OS".  If not, see <https://www.gnu.org/licenses/>

.. Présentation du noyau de Redox OS documentation master file, created by
   sphinx-quickstart

Présentation du noyau de Redox OS
=================================
Cette documentation constitue un parcours libre dans le micro-noyau


.. toctree::
   :maxdepth: 2
   :caption: Contents:

   a-classer/demarrage-noyau.rst
   processus/main.rst
   processus/creation.rst
   processus/interruptions.rst
   processus/IPC.rst
   processus/ordonnancement.rst
   processus/destruction.rst
   memoire/main.rst
   memoire/segmentation.rst
   memoire/pagination.rst
   memoire/retour-clone.rst
   messages/main.rst
   messages/evenements.rst
   messages/schemes.rst
   fichiers/main.rst
   fichiers/fuse.rst
   entrees-sorties/main.rst
   annexes/lexique.rst
   annexes/x86_64.rst
   annexes/PTI.rst
   annexes/fichiers.rst
   a-classer/noyau-utilisateur.rst
   a-classer/chargement.rst

Index
=====

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
